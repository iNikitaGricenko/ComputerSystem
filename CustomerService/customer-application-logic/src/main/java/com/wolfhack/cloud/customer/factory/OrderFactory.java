...
final class OrderFactory implements IOrderFactory, IOrderCreator {
	...
	public AnalyticsResponse create(List<CustomerOrder> customerOrders) throws ExecutionException, InterruptedException {
		...
		Callable<Double> totalPriceCallable = () -> orderItems.stream().mapToDouble(orderItem -> orderItem.getUnitPrice() * orderItem.getQuantity()).sum();
		Callable<Double> maxPriceCallable = () -> orderItems.stream().mapToDouble(orderItem -> orderItem.getUnitPrice() * orderItem.getQuantity()).max().orElse(0);
		Callable<Double> minPriceCallable = () -> orderItems.stream().mapToDouble(orderItem -> orderItem.getUnitPrice() * orderItem.getQuantity()).min().orElse(0);
