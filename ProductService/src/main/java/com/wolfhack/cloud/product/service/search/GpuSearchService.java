...
public class GpuSearchService implements GpuSearchServiceInterface {
	...
	@Override
	public long save(Product<Gpu> gpu) {
		GpuSearch searchModel = gpuMapper.toSearch(gpu);
		return gpuSearchRepository.save(searchModel).getId();
	}

	@Override
	public List<Product<Gpu>> findByTitle(String line, Pageable pageable) {
	...
	@AopLog
	@Override
	public List<Product<Gpu>> findByAllTextFields(String line, Pageable pageable) {
		String[] fields = Arrays.stream(Gpu.class.getFields()).filter(field -> field.getType().isInstance(String.class)).map(Field::getName).toArray(String[]::new);
	...
	@AopLog
	@Override
	public List<Product<Gpu>> findByAllFields(String line, Pageable pageable) {
		String[] fields = Arrays.stream(Gpu.class.getFields()).map(Field::getName).toArray(String[]::new);
	...
	@Override
	public long update(Product<Gpu> gpu, long id) {
