...
public class UserService implements IUserService {
    ...
    public User save(User user) {
        outputUser.findByLogin(user.getLogin())
    ...
    public User edit(User user) throws UserNotFoundException {
        return inputUser.save(outputUser.findById(user.getId())
                .map(foundUser ->
                        foundUser.renovator()
                                .update(user, User::getPassword, password ->
                                        user.setPassword(getPasswordEncoder().encode(password)))
                                .update(user, User::getRole, user::setRole)
                                .update(user, User::getPhoto, user::setPhoto)
                                .update(user, User::getLogin, user::setLogin)
                                .update(user, User::getUsername, user::setUsername)
                                .build())
