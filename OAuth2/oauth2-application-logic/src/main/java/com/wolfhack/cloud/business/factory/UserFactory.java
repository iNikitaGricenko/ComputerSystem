...
public class UserFactory implements IUserFactory {
	...
	public User edit(User user, User editor) {
		return user.renovator().update(editor, User::getPassword, password -> user.setPassword(getPasswordEncoder().encode(password))).update(editor, User::getRole, user::setRole).update(editor, User::getPhoto, user::setPhoto).update(editor, User::getLogin, user::setLogin).update(editor, User::getUsername, user::setUsername).build();
	}
